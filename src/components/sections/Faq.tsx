...
export default function Faq({ dict }: { dict: Dictionary }) {
  const f = dict.faq;
  return (
    <section id="faq" className="border-y border-white/5 bg-white/[0.015] py-20 sm:py-28">
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <Reveal delay={100}>
          <div className="glass mt-12 divide-y divide-white/10 rounded-2xl">
            {f.items.map((item) => (
              <details key={item.q} className="group px-6 py-5 open:bg-white/[0.03]">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-semibold text-white marker:content-none">
                  ...
                  <svg
                    ...
                    stroke="currentColor"
                    strokeWidth="2"
                    className="shrink-0 text-white/50 transition group-open:rotate-45 group-open:text-neon"
                  >
